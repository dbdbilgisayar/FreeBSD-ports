...
/*if(isset($argv[1])){*/
	...
	if ($telboy = 13) 
	{
		$sorgu2 = mysqli_fetch_array(mysqli_query($GLOBALS["___mysqli_ston"],"select * from Abone Where TcNo='".$telefon."' AND BitisT > '".$baslamat." ORDER BY CId DESC'"));
		$kontrol =  $sorgu2['CepTel']; 

		if ($_POST["telefon"] = $sorgu2['CepTel']) 
		{
		...
		} else 
		{
	        ...
	        $strMSG = "Dear internet user login with  username: ".$user." and password: ".$pass." Please login this username and password is coreecting ".$prm3." day. ";
	        $instance = new SMS_Api("5322040402", "asdXasd21");
   	        echo $instance->submit("CHRYSO", $strMSG, array($telefon));
