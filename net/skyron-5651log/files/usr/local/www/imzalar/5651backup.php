...
$pglinks = array("", "skyron5651log.php", "skyron5651log.php?lview=" . $lview, "@self");

include("head.inc");

$tab_array = array();
$tab_array[] = array(gettext("Log Goruntule"), false, "skyron5651log.php");
$tab_array[] = array(gettext("Ayarlar"), false, "skyron5651setup.php");
$tab_array[] = array(gettext("Cloud Backup"), false, "skyron5651backup.php");
display_top_tabs($tab_array, true);



$i_dizini="imzali_kayitlar";
$ni_dizini="nitelikli_imzali_kayitlar";
...
$languageStrings = Array(
	"en" => Array(
		...
		"[ geri ]" => "", // special name displayed for the .. folder
		"1 item" => "1 Nesne", // displayed when a subdirectory contains exactly one file or directory
		"%d items" => "%d Nesne", // 0 items, 42 items; displays the number of files and directories in a subdirectory. Leave %d as it is.
		...
		"Illegal characters detected in URL, ignoring." => "URL'de geçersiz karakter tespit edildi.", // displayed when an URL parameter contains HTML special characters
		"Illegal path specified, ignoring." => "Geçersiz dosya yolu.", // displayed when the path URL parameter contains a potentially dangerous path
		...
		"pages" => "Sayfa", // as in "4 pages"
		"previous" => "Geri", // as in "previous page"
		"next" => "Ileri" // as in "next page"
	)
		...
		foreach($displayColumns AS $column) {
			switch ($column) {
				...
				case "description":
					...
					//echo $files[$i]["filetype"];
					$hrn=explode('/',$_GET["path"]);
					//print $hrn[0];
if ($hrn[0]==$i_dizini){

					if ($files[$i]["filetype"]=="archive") {
						$file=$files[$i]["name"];
						$yol=$files[$i]["link"];
						echo "<a href=\"imza_dogrula.php?dosya=$yol&amp;t=1\">Dogrula</a>";
						
						$bugun=time();
						$hrn=explode('.',$file);
						$fdate=strtotime(substr($hrn[2],0,8));
						$datediff=floor(($bugun - $fdate)/(60*60*24));
	
						if ($datediff>=190){
							//$yol=str_replace('/','-',$yol);
							echo "&nbsp; <a href=\"log_temizle.php?dosya=$file&amp;t=1\" onclick=\"return window.confirm('Bu kayıt silinecek emin misiniz?')\";>Sil</a>";							
						}
					}
...
if ($hrn[0]==$ni_dizini){
					...
					if ($files[$i]["filetype"]=="archive") {
						...
						$hrn=explode('.',$file);
						if ($hrn[0]=="TRtrust"){
							echo "<a href=\"imza_dogrula.php?dosya=$yol&amp;t=2\">Dogrula</a>";
						}
						
						if ($hrn[0]=="TBTK"){
							$msg="Tubitak Imzali Logların Doğrulama İşlemi, Tubitak Tarafından Sağlanan Zamane Yazılımyla Yapılmaktadır.\n Lutfen Doğrulamak İstediğiniz Dosyayı Bilgisayarınıza İndirdikten Sonra Zamane Yazılımıyla Doğrulayınız";
							echo "<a href=\"javascript:alert('".$msg."');\">Dogrula</a>";
						}
						...
						if ($datediff>=190){
							//$yol=str_replace('/','-',$yol);
							echo "&nbsp; <a href=\"log_temizle.php?dosya=$file&amp;t=2\" onclick=\"return window.confirm('Bu kayıt silinecek emin misiniz?')\";>Sil</a>";							
						}
					}
