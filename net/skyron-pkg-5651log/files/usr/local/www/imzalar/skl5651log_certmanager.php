...
if ((!empty($act) &&
    ...
    !$thiscert) ||
    (($act == 'del') && empty($_POST))) {
	pfSenseHeader("skl5651log_certmanager.php");
	exit;
}
...
if ($_POST['save'] == gettext("Save")) {
	...
	if (!$input_errors) {
		...
		if ((isset($userid) && is_numeric($userid)) && !$input_errors) {
			post_redirect("skl5651log_usermanager.php", array('act' => 'edit', 'userid' => $userid));
			exit;
		}
	}
...
$pgtitle = array(gettext("5651log"), gettext("Certificate Manager"), gettext("Certificates"));
$pglinks = array("", "skl5651log_camanager.php", "skl5651log_certmanager.php");
...
$tab_array = array();
$tab_array[] = array(gettext("CAs"), false, "skl5651log_camanager.php");
$tab_array[] = array(gettext("Certificates"), false, "skl5651log_certmanager.php");
$tab_array[] = array(gettext("Certificate Revocation"), false, "skl5651log_crlmanager.php");
display_top_tabs($tab_array);

if (in_array($act, array('new', 'edit')) || (($_POST['save'] == gettext("Save")) && $input_errors)) {
	$form = new Form();
	$form->setAction('skl5651log_certmanager.php')->setMultipartEncoding();
	...
	if (!$internal_ca_count) {
		$section->addInput(new Form_StaticText(
			...
			sprintf(gettext('%1$sCreate%2$s an internal CA.'), '<a href="skl5651log_camanager.php?act=new&amp;method=internal"> ', '</a>')
		));
	} else {
		$allCas = array();
		foreach ($a_ca as $ca) {
	...
	$form = new Form(false);
	$form->setAction('skl5651log_certmanager.php?act=csr');
...
?>
				<tr>
					...
					<td>
						<?php if (!$cert['csr']): ?>
							<a href="skl5651log_certmanager.php?act=edit&amp;id=<?=$cert['refid']?>" class="fa fa-pencil" title="<?=gettext("Edit Certificate")?>"></a>
							<a href="skl5651log_certmanager.php?act=exp&amp;id=<?=$cert['refid']?>" class="fa fa-certificate" title="<?=gettext("Export Certificate")?>"></a>
							<?php if ($cert['prv']): ?>
								<a href="skl5651log_certmanager.php?act=key&amp;id=<?=$cert['refid']?>" class="fa fa-key" title="<?=gettext("Export Key")?>"></a>
							<?php endif?>
							<?php if (is_cert_locally_renewable($cert)): ?>
								<a href="skl5651log_certmanager_renew.php?type=cert&amp;refid=<?=$cert['refid']?>" class="fa fa-repeat" title="<?=gettext("Reissue/Renew")?>"></a>
							<a href="skl5651log_certmanager.php?act=p12&amp;id=<?=$cert['refid']?>" class="fa fa-archive" title="<?=gettext("Export P12")?>"></a>
							<?php endif ?>
						<?php else: ?>
							<a href="skl5651log_certmanager.php?act=csr&amp;id=<?=$cert['refid']?>" class="fa fa-pencil" title="<?=gettext("Update CSR")?>"></a>
							<a href="skl5651log_certmanager.php?act=req&amp;id=<?=$cert['refid']?>" class="fa fa-sign-in" title="<?=gettext("Export Request")?>"></a>
							<a href="skl5651log_certmanager.php?act=key&amp;id=<?=$cert['refid']?>" class="fa fa-key" title="<?=gettext("Export Key")?>"></a>
						<?php endif?>
						<?php if (!cert_in_use($cert['refid'])): ?>
							<a href="skl5651log_certmanager.php?act=del&amp;id=<?=$cert['refid']?>" class="fa fa-trash" title="<?=gettext("Delete Certificate")?>" usepost></a>
						<?php endif?>
					</td>
				</tr>
<?php
